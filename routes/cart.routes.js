...
CartRoute.post("/cart/add/:productId", async (req, res) => {
  try {
...
CartRoute.get("/cart", async (req, res) => {
  try {
    ...
    let cart = await CartModel.findOne({ user: userId }).populate({
      path: "items.product",
      model: ProductModel, // Specify the model
    });
...
CartRoute.put("/cart/update/:cartItemId", async (req, res) => {
  try {
    let { userId } = req.user_data;
...
CartRoute.delete("/cart/remove/:cartItemId", async (req, res) => {
  try {
    let { userId } = req.user_data;
    let cartItemId = req.params.cartItemId;
